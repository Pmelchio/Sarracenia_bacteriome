...
##All fastq files available 16S rDNA reads from this project and associated SRA metadata can be found at 
##www.ncbi.nlm.nih.gov under Bioproject PRJNA1059026, accession numbers SAMN39197316 – SAMN39197503

#setwd("path to working directory") and path to sequence files
...
#m. Track reads through dada2 pipeline, and create track.csv file (metadata) for phyloseq.  This 'track' file 
    #will need to have all  other metadata (from other sources) added to it before use downstream.
...
#c Calculate diversity matrices and add columns and data to meta data (sample_data(ps1)) data frame.  
   #Then create final ASV-level phyloseq object with all alpha div data.

meta_ASV <- data.frame(sample_data(ps_ASV.pie))
richASV <- estimate_richness(ps_ASV.pie, split = TRUE, measures = NULL)

#create columns and add into meta data
...
#Set revised metadata fram as the sample_data component of a PS object
meta_ASV = sample_data(meta_ASV)

#Create new PS object (merge)
physeq_NC_ASVs_alpha_LfYr703 = merge_phyloseq(ps_ASV.pie, meta_ASV)
saveRDS(physeq_NC_ASVs_alpha_LfYr703),"/physeq_NC_ASVs_alpha_LfYr703.rds")

#d Calculate diversity matrices and add create final Family-level ps object with all alpha div data.
   #This is a repeat of the previous code to add columns.
...
#Create and save new family-level phyloseq object
ps.fam.1 = merge_phyloseq(ps.fam, metafam)
saveRDS(ps.fam.1, file = "/physeq.family_NC_alpha726.rds")
...
#a.  Read in ps object, convert sample data to data.frame 

ps1 <- readRDS("physeq_NC_ASVs_alpha_LfYr703.rds")  #For ASV level analysis
ps.fam.1 <- readRDS("physeq.family_NC_alpha726.rds")  #For family level analysis

#Create data frames of metadata components
...
#b. Final Models at ASV taxon level (model iteration testing code can be delivered upon request)
    #Note:  ASV alpha analysis was not presented in manuscript.  Only the family level analysis was used (below)
...
#c.  Final Models at Family taxon level (Table 2)
...
##Note:  In manuscript, Sample_Date_No, Bog_No., LeafYr, qPCR_log10, and LeafID are referred (for clarity and readability) 
...
#################################
#Part V:  Plotting Alpha Diversity Metrics (Fig 2 a - f)
...
##Read in files and create ASF and Fam level metadata frams as in above sections if needed:
...
##PART VI: Analysis of Beta Diversity
...
#VI.1. Ordinate (NMDS) for Bray-Curtis at ASV and family level
...
#b. Plot NMDS Ordinations by bog number and sample date (Figure 3)
...
#VI.2 Statistical Analysis: Create Bray-Curtis distance matrices for each taxon (Bray-Curtis distance matrices) 
...
##PART VII:  Differential Abundance Testing and Plots

#VII.1  CSS normalization (metagenomeSeq package) of count data for diff abundance testing; save new phyloseq object

library(metagenomeSeq)
library(Maaslin2)
library(dplyr)

##Read in files and create ASF and Fam level metadata frames as in above sections if needed:

ps1 <- readRDS("physeq_NC_ASVs_alpha_LfYr703.rds")  

#a. Normalize data and create new phyloseq object (ASV Level).  Note: New CSS PS object used to agglom to family and genus again.
...
#VII.2.  Differential Abundance Testing with Maaslin2 (family level)
...
#VII.3.  Differential Abundance Testing with Maaslin2 (genus level)
...
#VII.4:  Plotting Differential Abundance Results (Fig 6 a-b)
...
#VIII.3: Plotting Relative abundance (Fig 4 a - b)
...
#c.  Plot prevalence - diazotrophic genera (Fig 5)
...
################################################################################################################
#PART IX:  Analysis of qPCR data for nifH gene quantification (Fig 7)

library(lme4)
library(emmeans)

##Notes:  I extracted the metadata (sample_data) component of the physeq_NC_ASVs_alpha_LfYr703.rds PS object and saved it as 
##the file:  meta9.csv. Then I used MS Excel to add in adjusted/normalized nifH quantification values, and exported back to R as
##'meta10' in the form of a data frame. 

meta10 <- read.csv("meta9_final.csv")

#a. Model relationships between date, bog, etc vs.nifH concentrations
